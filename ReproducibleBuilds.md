...
```
user@host:$ adb pull $(adb shell pm path org.privately.chat | grep /base.apk | awk -F':' '{print $2}') ~/reproducible-signal/apk-from-google-play-store/Signal-$(adb shell dumpsys package org.privately.chat | grep versionName | awk -F'=' '{print $2}').apk
```
...
```
user@host:$ adb shell pm path org.privately.chat
```
This will output something like:
```
package:/data/app/org.privately.chat-aWRzcGlzcG9wZA==/base.apk
```

The output will tell you where the Signal APK is located in your device. (In this example the path is `/data/app/org.privately.chat-aWRzcGlzcG9wZA==/base.apk`)
...
```
user@host:$ adb pull \
              /data/app/org.privately.chat-aWRzcGlzcG9wZA==/base.apk \
